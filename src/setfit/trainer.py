...
class Trainer:
    ...
    def __init__(
        ...
        embedding_body_args: Optional[BodyTrainingArguments] = None,
        classifier_body_args: Optional[BodyTrainingArguments] = None,
        classifier_head_args: Optional[HeadTrainingArguments] = None,
        ...
        self.embedding_body_args = embedding_body_args
        self.classifier_body_args = classifier_body_args
        self.classifier_head_args = classifier_head_args
    ...
    def freeze(self):
        ...
        """
        if not isinstance(self.model.head, torch.nn.Module):
    ...
    def train(
        ...
        self.train_embeddings(x_train, y_train, embedding_body_args)
        self.train_classifier(x_train, y_train, classifier_body_args, classifier_head_args)

    def train_embeddings(
        self, x_train: List[str], y_train: List[int], embedding_body_args: Optional[BodyTrainingArguments] = None
    ):
        embedding_body_args = (
            embedding_body_args or self.embedding_body_args or self.model.body.args_class.embeddings_default()
        )

        self.model.body.fit(x_train, y_train, **embedding_body_args.to_dict())
    ...
    def train_classifier(
        ...
        classifier_body_args = (
            classifier_body_args or self.classifier_body_args or self.model.body.args_class.classifier_default()
        )
        classifier_head_args = (
            classifier_head_args or self.classifier_head_args or self.model.head.args_class.classifier_default()
        )

        if not classifier_head_args.end_to_end:
            embeddings = self.model.encode(x_train)
            self.model.head.fit(embeddings, y_train)
        else:
            self.model.fit(
                x_train,
                y_train,
                body_learning_rate=classifier_body_args.learning_rate,
                **classifier_head_args.to_dict(),
            )
    ...
    def evaluate(self):
        ...
        if isinstance(self.metric, str):
            metric_config = "multilabel" if self.model.head.multi_target_strategy is not None else None
